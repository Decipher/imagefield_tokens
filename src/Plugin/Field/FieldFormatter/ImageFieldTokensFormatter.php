...
class ImageFieldTokensFormatter extends ImageFormatter {
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    foreach ($files as $delta => $file) {
      ...
      $request_params = \Drupal::routeMatch()->getParameters()->all();
      $entity = array_pop($request_params);
      $entity_type = $entity->getEntityTypeId();
      // Replace entity tokens.
      $alt_token = \Drupal::token()->replace($item_values['alt'], [$entity_type => $entity]);
      $title_token = \Drupal::token()->replace($item_values['title'], [$entity_type => $entity]);
